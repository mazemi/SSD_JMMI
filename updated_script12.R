...

{
  ...
  # detect state rows for some sheets line "Table - Median all" and "Table - Stock"
  ...
  # optional but recommended: remove empty columns
  # median.etb.full <- median.etb.full[, colSums(!is.na(median.etb.full)) > 0]
  ...
  # Convert columns 4 to the last column to numeric
  ...
  writeData(wb2, "feedback_availability", "Available", colNames = FALSE, startRow = 1, startCol = 4)
  writeData(wb2, "feedback_availability", "Unavailable", colNames = FALSE, startRow = 1, startCol = 44)
  ...
  colnames(min.tmp.etb) <- gsub("_price_unit", "", colnames(min.tmp.etb))
  colnames(max.tmp.etb) <- gsub("_price_unit", "", colnames(max.tmp.etb))

  min_col_names.etb <- colnames(min.tmp.etb)
  max_col_names.etb <- colnames(max.tmp.etb)
  item_col_names.etb <- min_col_names.etb[!min_col_names.etb %in% location.headers]
  min_col_names.etb <- min_col_names.etb[!min_col_names.etb %in% location.headers]
  max_col_names.etb <- max_col_names.etb[!max_col_names.etb %in% location.headers]

  min_col_names.etb <- paste("min", min_col_names.etb, sep = ".")
  new_col_names.etb <- cbind(min_col_names.etb, paste("max", max_col_names.etb, sep = "."))

  max.tmp2.etb <- max.etb %>%
    ungroup() %>%
    select(-c("state", "county", "location"))

  min.max.etb <- cbind(min.etb, max.tmp2.etb)
  colnames(min.max.etb) <- c(location.headers, new_col_names.etb)

  for (col in names(min.max.etb)) {
    min.max.etb[[col]][is.infinite(min.max.etb[[col]])] <- NA_real_
  }

  for (i in 1:length(item_col_names.etb)) {
    min_col.etb <- paste0("min.", item_col_names.etb[i])
    max_col.etb <- paste0("max.", item_col_names.etb[i])
    min.max.etb[[paste0("diff.", item_col_names.etb[i])]] <- round(
      (min.max.etb[[max_col.etb]] - min.max.etb[[min_col.etb]]) / min.max.etb[[min_col.etb]], 3
    )
  }

  new_column_order.etb <- vector()

  for (i in 1:length(item_col_names.etb)) {
    new_column_order.etb <- c(new_column_order.etb, paste0("min.", item_col_names.etb[i]))
    new_column_order.etb <- c(new_column_order.etb, paste0("max.", item_col_names.etb[i]))
    new_column_order.etb <- c(new_column_order.etb, paste0("diff.", item_col_names.etb[i]))
  }

  # Reorder the data frame columns based on the new order
  min.max.etb <- min.max.etb[, c(location.headers, new_column_order.etb)]
  new_col_names0.etb <- names(min.max.etb)[4:ncol(min.max.etb)]

  # Iterate over the column names and add prefixes
  for (col_name in new_col_names0.etb) {
    if (startsWith(col_name, "min.")) {
      new_name.etb <- "min"
    } else if (startsWith(col_name, "max.")) {
      new_name.etb <- "max"
    ...
    # Rename the column
    names(min.max.etb)[which(names(min.max.etb) == col_name)] <- new_name.etb
  }
  ...
  livestock.items.name <- c("Goat", "Chicken")

  services.items.name <- c("Milling.costs")
  ...
  for (i in state_rows) {
    addStyle(wb2, sheet = "Table - Indices", style = seperator_style, rows = i, cols = 12:19)
  }
...
# add en empty column for water.
result.table$`Water (1 L)` <- NA
