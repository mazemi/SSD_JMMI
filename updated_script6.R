...
# Export all the dataframes and save themm to the analysis workbook:
...
removeFilter(wb, "expectation_price")
removeFilter(wb, "all_prices")
...
deleteData(wb, sheet = "quotation_raw", rows = 1:100, cols = 1:50, gridExpand = TRUE)
...
deleteData(wb, sheet = "all_prices_wholesale", rows = 1:2000, cols = 1:100, gridExpand = TRUE)
writeData(wb, sheet = "all_prices_wholesale", all_prices.wholesale, colNames = TRUE)
...
deleteData(wb, sheet = "quotation_feedback", rows = 1:100, cols = 1:100, gridExpand = TRUE)
writeData(wb, sheet = "quotation_feedback", feedback.quotation, colNames = TRUE)

deleteData(wb, sheet = "feedback_availability_raw", rows = 3:101, cols = 1:200, gridExpand = TRUE)
...
deleteData(wb, sheet = "insecurity_market", rows = 1:100, cols = 1:100, gridExpand = TRUE)
writeData(wb, sheet = "insecurity_market", insecurity_marketplace, colNames = TRUE, rowNames = FALSE)
...

{
  ...
  median.df <- median2[, 4:ncol(median2)]
  median.df <- median.df[, median.col.order]
  
  median.overall <- median.df %>% summarise_all(~ median(as.numeric(.), na.rm = TRUE))
  ...
  median2 <- median2 %>% rowwise() %>%  mutate(
    Food.price.index = sum(
      ...
      if_else(!is.na(Cooking.oil), Cooking.oil, get_default_value(median.overall, "Cooking.oil")), na.rm = TRUE
    ),
    ...
    Covid.price.index = sum(
      ...
      if_else(!is.na(Rice), Rice, get_default_value(median.overall, "Rice")),
      if_else(!is.na(Sugar), Sugar, get_default_value(median.overall, "Sugar")),
      if_else(!is.na(Soap), Soap, get_default_value(median.overall, "Soap")), na.rm = TRUE
    )
  )
...
# final result:
file.path <- "update.file2.xlsx"
options("openxlsx.borderColour" = "#343436")
options("openxlsx.borderStyle" = "thin")

wb2 <- createWorkbook()

freez_pane <- function(sheet_name, row_number, column_number){
  freezePane(wb2, sheet = sheet_name, firstActiveRow = row_number, firstActiveCol = column_number)
}

gray_style1 <- createStyle( halign = "center", valign = "center", fgFill = "#a2a2a2", border = "TopBottomLeftRight")
gray_style2 <- createStyle( halign = "center", valign = "center", fgFill = "#d2d2d2", border = "TopBottomLeftRight")

addWorksheet(wb2, "median")
...
writeData(wb2, "median", t(c(median.col.order, median.col.indexs, median.col.extra)), colNames = FALSE, startRow = 1, startCol = 4)
...
addStyle(wb2, "median", style = gray_style1, rows = 1, cols = 1:ncol(median.overall) + 3, gridExpand = TRUE)
addStyle(wb2, "median", style = gray_style2, rows = 2:4, cols = 3, gridExpand = TRUE)
addStyle(wb2, "median", style = gray_style1, rows = 1, cols = 1:3, gridExpand = TRUE)
freez_pane("median", 5, "D")
setColWidths(wb2, "median", cols = 1:3, widths = c(20, 20, 20)) 

# bold_style <- createStyle(textDecoration = "bold")
# addStyle(wb2, sheet = "median", style = bold_style, rows = 1, cols = 1:ncol(median2))
# addStyle(wb2, sheet = "median USD", style = bold_style, rows = 1, cols = 1:ncol(median.usd))

addStyle(wb2, "median USD", style = gray_style1, rows = 1, cols = 1:ncol(median.usd.overall) + 3, gridExpand = TRUE)
addStyle(wb2, "median USD", style = gray_style2, rows = 2, cols = 3, gridExpand = TRUE)
addStyle(wb2, "median USD", style = gray_style1, rows = 1, cols = 1:3, gridExpand = TRUE)
freez_pane("median USD", 3, "D")
setColWidths(wb2, "median USD", cols = 1:3, widths = c(20, 20, 20)) 

addWorksheet(wb2, "minmax")
...
addStyle(wb2, "minmax", style = gray_style1, rows = 1, cols = 4:ncol(min.max), gridExpand = TRUE)
addStyle(wb2, "minmax", style = gray_style2, rows = 2, cols = 1:ncol(min.max), gridExpand = TRUE)
...
median.wholesale.overall <- jmmi %>%
  select(sorghum_grain_wholesale_price_unit_ssp, maize_grain_wholesale_price_unit_ssp, 
...
addWorksheet(wb2, "median wholesale")
...
writeData(wb2, "median wholesale", median.wholesale, colNames = TRUE, startRow = 5, startCol = 1)
setColWidths(wb2, "median wholesale", cols = 1:7, widths = c(20, 20, 20, 17, 17, 17, 17))
addStyle(wb2, "median wholesale", style = gray_style1, rows = 1, cols = 4:7, gridExpand = TRUE)
addStyle(wb2, "median wholesale", style = gray_style1, rows = 5, cols = 1:7, gridExpand = TRUE)
addStyle(wb2, "median wholesale", style = gray_style2, rows = 2:4, cols = 3, gridExpand = TRUE)
freez_pane("median wholesale", 6, "D")
...
addWorksheet(wb2, "road")
writeData(wb2, "road", road2, colNames = TRUE, startRow = 1, startCol = 1)
addStyle(wb2, "road", style = gray_style1, rows = 1, cols = 1:ncol(road2), gridExpand = TRUE)
freez_pane("road", 2, "A")
setColWidths(wb2, "road", cols = 1:ncol(road2), widths = c(35, 17, 17, 22, 22, 17))  
...
addWorksheet(wb2, "border")
writeData(wb2, "border", border2, colNames = TRUE, startRow = 1, startCol = 1)
addStyle(wb2, "border", style = gray_style1, rows = 1, cols = 1:ncol(border2), gridExpand = TRUE)
freez_pane("border", 2, "A")
setColWidths(wb2, "border", cols = 1:ncol(border), widths = c(35, 14, 14, 22, 25, 14)) 
saveWorkbook(wb2, file.path, overwrite = TRUE)
